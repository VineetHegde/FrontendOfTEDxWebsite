...
const TicketPage = () => {
  ...
  const initiatePayment = async () => {
    try {
      ...
      // ENHANCED: Get fresh availability immediately before payment
      const freshAvailabilityRes = await fetch(`${API_BASE_URL}/api/payment/availability`);
      if (!freshAvailabilityRes.ok) {
        alert("❌ Unable to check availability. Please try again.");
        setShowModal(false);
        return;
      }

      const freshAvailability = await freshAvailabilityRes.json();
      const sessionKey = SESSION_KEY[selectedSession.name];
      let available = 0;

      switch (sessionKey) {
        case "morning":
          available = freshAvailability.morningAvailable;
          break;
        case "evening":
          available = freshAvailability.eveningAvailable;
          break;
        case "fullDay":
          available = freshAvailability.fullDayAvailable;
          break;
      }

      console.log("🔍 Fresh availability check:", {
        session: selectedSession.name,
        available,
        freshAvailability
      });

      // Block payment if no seats available
      if (available <= 0) {
        alert(`🚫 Sorry! The ${selectedSession.name} is now sold out. Please select a different session.`);
        setShowModal(false);
